...
7. **Assemble and save final file**:
   ```bash
   OUTPUT_DIR=$(python3 ~/.claude/skills/yt-transcript/yt_transcript_utils.py load-config | python3 -c "import sys,json; print(json.load(sys.stdin)['output_dir'])")
   CLEAN_TITLE=$(python3 ~/.claude/skills/yt-transcript/yt_transcript_utils.py sanitize-filename "$TITLE")
   DATE=$(date +%Y-%m-%d)

   python3 ~/.claude/skills/yt-transcript/yt_transcript_utils.py assemble-final \
   ...
   **↳ WRITE State**: 更新 `step: 5`, `last_action: saved final file`
...
> [!IMPORTANT]
> Do NOT read the optimized text file into context. Only review the JSON report from the script.

**If `"passed": false`**, review the `warnings` array and STOP. Review Step 4.

**↳ WRITE State**: 更新 `step: 4`, `last_action: optimized, quality verified`
...
> The Agent does NOT read the optimized text into context. The script handles file assembly.

```bash
OUTPUT_DIR=$(grep 'output_dir' /tmp/${VIDEO_ID}_state.md | sed 's/.*: *//')

# If OUTPUT_DIR is empty, reload from config
